...
def calculate_house(selections):
    ...
    #loop through each answer and assign points to houses
    for question_id, answer_house in selections.items():
        house_points[answer_house] += 1
    
    #find house with the most points
    max_points = 0 
    chosen_house = ""
    ...
    #handle tie
    if chosen_house == "":
        chosen_house = "Gryffindor" #default house
...
def index():
    try:
        with open('Potter2.1_full_quiz.html', 'r') as f:
            return f.read()
...
def submit():
    ...
    if request.is_json:
        ...
        #extract from form data if not JSON
        selections = {}
        for question, house in request.form.items():
            selections[question] = house
    
    #calculate house
    house = calculate_house(selections)
